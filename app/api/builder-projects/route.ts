...
import { builderProjects, users } from "../../../db/schema";
...
async function projectState(request: Request) {
  ...
  const [ownRows, reviewRows, people] = await Promise.all([
    ...
    student.role === "owner" ? db.select({ id: users.id, username: users.username, displayName: users.displayName }).from(users) : Promise.resolve([]),
  ]);
  const shape = (row: typeof builderProjects.$inferSelect) => ({ ...row, milestones: parseMilestones(row.milestones) });
  return {
    role: student.role,
...
export async function POST(request: Request) {
  try {
    ...
    const [existing] = id ? await db.select().from(builderProjects).where(and(eq(builderProjects.id, id), eq(builderProjects.userId, student.id))).limit(1) : [];
    if (id && !existing) return Response.json({ error: "Build project not found" }, { status: 404 });
    if (action === "submit") {
