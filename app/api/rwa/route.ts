...
async function ensureStarterAssets(db: Awaited<ReturnType<typeof requireCampusUser>>["db"]) {
  for (const asset of starterAssets) await db.insert(rwaAssets).values(asset).onConflictDoUpdate({ target: rwaAssets.id, set: { annualYieldBps: asset.annualYieldBps, incomeModel: asset.incomeModel } });
}
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "create") {
      ...
      const annualYieldPercent = Number(body.annualYieldPercent);
      ...
      if (!Number.isFinite(annualYieldPercent) || annualYieldPercent < 0 || annualYieldPercent > 30) return Response.json({ error: "The simulated annual return must be between 0% and 30%" }, { status: 400 });
      ...
      await db.insert(rwaAssets).values({ id, creatorUserId: student.id, name, symbol, category, description, rights, incomeModel, risk, totalUnits, priceCredits, annualYieldBps: Math.round(annualYieldPercent * 100) });
    ...
    if (action === "claim_income") {
      ...
      const amountCredits = Math.round((holding.units * asset.priceCredits * asset.annualYieldBps) / 10_000 / 12);
