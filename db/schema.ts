...
export const xpProofs = sqliteTable("xp_proofs", {
  ...
  missionType: text("mission_type", { enum: ["lesson", "campaign_buy"] }).notNull(),
  chain: text("chain", { enum: ["ethereum", "robinhood"] }).notNull().default("ethereum"),
...
export const partnerLabTeams = sqliteTable("partner_lab_teams", {
  ...
  name: text("name").notNull(),
  characterKey: text("character_key").notNull().default("pending"),
  ...
  tokenPitch: text("token_pitch").notNull().default(""),
  initialBuyEth: text("initial_buy_eth").notNull().default("0"),
  ...
  graduationTxHash: text("graduation_tx_hash"),
  feedbackReference: text("feedback_reference"),
...
export const partnerLabProofs = sqliteTable("partner_lab_proofs", {
  ...
  teamId: text("team_id").notNull().references(() => partnerLabTeams.id, { onDelete: "cascade" }),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  proofType: text("proof_type", { enum: ["launch", "buy", "sell", "graduation", "feedback"] }).notNull(),
  transactionHash: text("transaction_hash"),
